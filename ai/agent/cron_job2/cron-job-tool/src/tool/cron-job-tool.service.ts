...
export class CronJobToolService {
    ...
    constructor() {
        const cronJobArgsSchema = z.object({
            ...
            id: z.string().optional().describe('任务ID，（toggle时需要）'),
            enabled: z.boolean().optional().describe('是否启用任务（toggle时可选）；不传则自动取反'),
            type: z.enum(['cron', 'every', 'at'])
                .optional().describe(`任务类型（add时需要）: cron（按Cron表达式循环执行）/ every
                    （按固定间隔毫秒循环执行）/ at（在指定时间点执行一次，执行后自动停用）`),
            instruction: z.string().optional().describe(`任务说明/指令 （add时需要）。要求： \n 
                1)从用户自然语言中去掉"什么时候执行"的定时部分后，保留纯粹要执行的任务内容\n 
                2)必须是自然语言描述， 不能工具调用或代码（例如不能写send_mail(...)/db_users_crud(...)
                /web_search(...)）。\n 
                3)不要擅自补全细节或改写脚本。
                `),
            cron: z.string().optional().describe('Cron表达式(type=cron时需要，例如 */5 * * * * *)'),
            everyMs: z.number().int().positive().optional().describe('固定间隔毫秒（type=every 时需要，例如 60000 表示每分钟执行一次）'),
            at: z.string().optional().describe(`指定触发时间点 （type=at 时需要，ISO字符串，例如
                2026-03-18T12:34:56.000Z;到点执行一次后自动停用）`),
        });
        this.tool = tool(
            async(
                {
