...
- 五种属性值 准确、简洁
    ...
    - absolute 相对最近的非static 祖先定位
         如果没有，那么相对body 定位
    - fixed 相对于视窗定位 
    - sticky    粘性定位 是一种css定位方式，它让元素在滚到到
    特定阈值前表现得像相对定位,到达阈值后固定在视口中，实现类似吸顶或吸附的效果。

- 业务场景
    - 结合relative 和 absolute 消息提醒，在右上角。
...
- 底层
    ...
    - 独立图层渲染
    absolute ? + ?
- 适当使用transform:translated(0,0,0);
...
- position: fixed 如果在transform: translateZ(0)下面，会失效
    transform 会有一个新的包含块 fixed 不再相对于视口定位，而是相对于这个transform容器。
